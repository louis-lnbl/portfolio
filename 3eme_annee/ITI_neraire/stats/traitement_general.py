...
def syncronisation(df_gps: pd.DataFrame, df_imu: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
    ...
    df_imu['UTC'] = pd.to_datetime(df_imu['UTC'], utc=True, errors='raise')
...
def afficher_difference_kalman(df):
	plt.figure(figsize=(10, 10))
	plt.plot(np.abs(df["positions_pred_x"] - df["pos_reel_x"]), label="x")
	plt.plot(np.abs(df["positions_pred_y"] - df["pos_reel_y"]), label="x")
	plt.plot(np.abs(df["positions_pred_z"] - df["pos_reel_z"]), label="x")
	plt.legend()
	plt.show()

def afficher_sortie_strava(path_gps):
